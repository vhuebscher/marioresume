...
function MarioGame() {
  ...
  that.bindKeyPress = function() {
    ...
    canvas.addEventListener('touchend', function(e) {
      ...
      for (var i = 0; i < touches.length; i++) {
        ...
        if (touches[i].pageX > 200 && touches[i].pageX <= 450) {
          keys[39] = false;
        }
        if (touches[i].pageX > 450 && touches[i].pageX <= 550) {
          keys[16] = false;
          keys[17] = false;
        }
        if (touches[i].pageX > 550 && touches[i].pageX < 1280) {
          keys[32] = false;
        }
      }
    });
  ...
  this.gameOver = function() {
    ...
    gameUI.writeText('GAME OVER', centerPos - 260, height - 300);
    gameUI.writeText('YOU WIN MY RESUME', centerPos - 315, height / 2);
  };
